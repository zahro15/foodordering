<?php
// ======================================================================
// BAGIAN PHP INI SAMA SEKALI TIDAK SAYA UBAH. FUNGSI HAPUS AMAN.
...
if (!empty($produk_dibeli)) {
  foreach ($produk_dibeli as $produk) {
    ...
    // ====================================================================
    // PERBAIKAN PHP DI SINI: Menambahkan 'img' agar bisa ditampilkan
    $produk_dengan_total[] = [
        'nama' => $produk['name'] ?? '-', 
        'size' => $produk['size'] ?? '-', 
        'topping' => $produk['topping'] ?? [], 
        'qty' => $qty, 
        'subtotal' => $subtotal,
        'img' => $produk['img'] ?? '' // <- INFO GAMBAR SEKARANG DIBAWA
    ];
    // ====================================================================
  }
}
...
<style>
  ...
  /* ========================================================= */
  /* PERBAIKAN CSS DI SINI UNTUK MENAMPUNG GAMBAR */
  .item-card {
    display: flex;
    align-items: center; /* Membuat gambar dan tulisan sejajar di tengah */
    gap: 15px; /* Memberi jarak antara gambar dan tulisan */
  ...
  .item-details {
    flex-grow: 1; /* Membuat detail tulisan mengisi sisa ruang */
  }
  ...
  .promo-options label { margin-right: 15px; }
  .summary-box {
...
<div class="checkout-page">
  <div class="checkout-container">
    ...
    <form method="POST" id="checkout-form" action="index3.php">
      ...
      <?php if (!empty($produk_dengan_total)): ?>
        ...
        <div class="section">
          <?php foreach ($produk_dengan_total as $i => $produk): ?>
            <!-- ========================================================= -->
            <!-- PERBAIKAN HTML DI SINI: Menambahkan tag <img> -->
            <div class="item-card">
              <img src="../<?= htmlspecialchars($produk['img']) ?>" alt="<?= htmlspecialchars($produk['nama']) ?>" class="item-card-img">
